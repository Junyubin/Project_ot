...
def get_queries(start_date, end_date, query_type=None):
    ...
    curs = conn.cursor()
    if query_type == 'l':
        sql = "select ruleId, ruleQuery from motie_rule_single where ruleGubn = 2 and (state != 'D' and state != 'DE')"
        time = 'loged_time'
    elif query_type == 'p':
        sql = "select ruleId, ruleQuery from motie_rule_single where ruleGubn = 1 and (state != 'D' and state != 'DE')"
        time = 'packet_time'
    else:
    ...
    conn.close()
    log_list = []
    new_where = "where parseDateTimeBestEffortOrZero(substring(toString(time),1,14)) >= '{start_date}' and parseDateTimeBestEffortOrZero(substring(toString(time),1,14)) <= '{end_date}' and".format(start_date=start_date, end_date=end_date)
...
#             new_query = new_query.replace('dti.motie_lgsys_L005', 'dti.sect_lgsys_L005')
#             new_query = new_query.replace('dti.motie_lgsys_L009', 'dti.sect_lgsys_L009')
#             new_query = new_query.replace('dti.motie_lgsys_L011', 'dti.sect_lgsys_L011')
...
def get_corr():
    conn = pymysql.connect(host = config['mysql']['host'], port = config['mysql']['port'], user = config['mysql']['user'], password = config['mysql']['password'], db = config['mysql']['db'])
    curs = conn.cursor()
    log_sql = "SELECT multiId, ruleId from dti.motie_rule_mapping where ruleGubn = 2 and (state != 'D' and state != 'DE')"
    packet_sql = "SELECT multiId, ruleId from dti.motie_rule_mapping where ruleGubn = 1 and (state != 'D' and state != 'DE')"
...
def sect_rmse(y_true, y_pred, axis=None):
