...
#     def __create_neural_network(self):          
#         input_layer = Input(shape=(self.x_datashape[1:]))
#         encoder_1 = Conv2D(8, (2,1), activation='relu', padding='same')(input_layer)
#         max_pool_1 = MaxPooling2D((1, 2), padding='same')(encoder_1)
#         encoder_2 = Conv2D(16, (2,1), activation='relu', padding='same')(max_pool_1)
#         max_pool_2 = MaxPooling2D((2, 2), padding='same')(encoder_2)
        
#         decoder_1 = Conv2D(16, (2,2), activation='relu', padding='same')(max_pool_2)
#         up_sample_1 = UpSampling2D((1, 2))(decoder_1)
#         decoder_2 = Conv2D(8, (2,2), activation='relu', padding='same')(up_sample_1)
#         up_sample_2 = UpSampling2D((1, 2))(decoder_2)
        
#         reshape_1 = Flatten()(up_sample_2)
#         reshape_2 = Dense(self.x_datashape[-1] * self.x_datashape[-2] * self.x_datashape[-3], activation= 'sigmoid')(reshape_1)
#         reshape_3 = Reshape(self.x_datashape[1:])(reshape_2)

#         return Model(inputs=input_layer, outputs=reshape_3)
...
#             self.neural_network.compile(optimizer=self.optimizer, loss= loss_fn)
...
#         return self.__rmse_custom(Y, y_pred, axis=1).numpy() 
...
class lstm_model:
    def __init__(self, config=None, mode=None, name=None):
        ...
        self.name = name
        self.save_path_model = pwd + '/{}/{}'.format(config["common"]["path"], self.name)
#         self.save_path_model = os.path.abspath(os.getcwd())+'{}/{}'.format(config["common"]["path"], self.name)
        if not os.path.exists(self.save_path_model):
            os.makedirs(self.save_path_model)
